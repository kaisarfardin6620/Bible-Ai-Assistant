...
# This function is duplicated here for demonstration purposes to make the file self-contained.
# In a real backend, you would typically import this from a shared utility module (e.g., preachly_backend.py).
...
# --- Demonstration of Backend Usage (for testing purposes) ---
async def main_demonstration():
    """
    This asynchronous function demonstrates how 'get_daily_encouragement_verse'
    would typically be called within a backend application (e.g., by a web server).
    It simulates receiving user preferences and then calls the service function.
    """
    print("\n--- Backend Service Demonstration ---")
    
    # Simulate user preferences that would come from a database, user session, or API request.
    mock_user_preferences = {"bible_version": "NIV"}
    print(f"Simulating request for user with preferences: {mock_user_preferences}")
    
    # Call the main service function and await its result.
    verse_info = await get_daily_encouragement_verse(mock_user_preferences)
    
    # Print the results or handle errors as a backend would.
    if verse_info.get("error"):
        print(f"\nError encountered: {verse_info['error']}")
        # In a real backend, you might return an HTTP 500 error or a more specific error response.
    else:
        print(f"\nSuccessfully retrieved Daily Encouragement Verse ({verse_info['version']}):")
        print(f"Reference: {verse_info['reference']}")
        print(f"Text: {verse_info['verse_text']}")
        print(f"Summary: {verse_info.get('summary', '(No summary generated due to an issue.)')}")
    
    print("\n--- End of Demonstration ---")

# --- Entry Point for Local Testing ---
if __name__ == "__main__":
    # This block allows you to run the demonstration directly when executing the script.
    # In a production backend, this part would be replaced by your web framework's entry point.
